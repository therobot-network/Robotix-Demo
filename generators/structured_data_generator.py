    ...
    def generate_employee_data(self):
        ...
        for emp in EMPLOYEES:
            ...
            employee_data = {
                ...
                "manager": random.choice([e['name'] for e in EMPLOYEES if e['dept'] == emp['dept'] and ('VP' in e['title'] or 'Director' in e['title'])] or ["Sarah Chen"]),
...
Make it technical yet appealing, mentioning key features and target applications. Be specific and realistic for industrial/collaborative robotics."""
    ...
    def generate_product_data(self):
        ...
        print(f"   [{self._get_timestamp()}] Generating product descriptions..." + (" (AI-enhanced)" if self.use_ai else ""))
        ...
        for category, product_list in PRODUCT_CATEGORIES.items():
            for product_name in product_list:
                ...
                self._print_progress_bar(current_product - 0.5, total_products, "Product Catalog")
                ...
                product = {
                    ...
                    "payload_capacity": f"{random.randint(5, 100)} kg" if "Robot" in category and "Mobile" not in category else "N/A",
                    "reach_mm": f"{random.randint(600, 2000)} mm" if "Industrial" in category or "Collaborative" in category else "N/A",
                    "color_options": "Industrial Gray/Safety Yellow" if "Robot" in category else "N/A",
                    "configurations_available": random.choice(["Standard/Extended Reach", "Standard/Heavy Duty", "Single/Dual Arm", "Standard Only"]),
                    "inventory_on_hand": random.randint(5, 50) if category != "Software" else "Digital",
                    "inventory_location": random.choice([loc['city'] for loc in LOCATIONS if loc['type'] in ['Distribution Center', 'Manufacturing']]) if category != "Software" else "N/A",
                    "reorder_point": 10 if category != "Software" else "N/A",
                    "supplier": random.choice(["Robotix Manufacturing", "Global Automation Components", "Servo Systems Inc", "Vision Tech Ltd"]),
                    "lead_time_days": random.randint(30, 90) if "Robot" in category else random.randint(14, 45),
                    "status": "Active",
                ...
                sku_counter += 1
                self._print_progress_bar(current_product, total_products, "Product Catalog")
    ...
    def generate_customer_data(self, num_customers=500):
        ...
        company_prefixes = ["Advanced", "Precision", "Global", "Industrial", "Smart", "Auto", "Tech", "Innovative", "Quality", "Elite"]
        company_types = ["Manufacturing", "Automation", "Systems", "Industries", "Solutions", "Technologies", "Robotics", "Assembly"]
        ...
        for i in range(num_customers):
            ...
            customer = {
                ...
                "city": random.choice(["Seattle", "Portland", "Denver", "Austin", "Minneapolis", "Boston", "Detroit", "Chicago"]),
                "state": random.choice(["WA", "OR", "CO", "TX", "MN", "MA", "MI", "IL"]),
                "zip_code": f"{random.randint(10000, 99999)}",
                "signup_date": signup_date.strftime("%Y-%m-%d"),
                "customer_type": random.choice(["Manufacturing", "Warehouse/Logistics", "Research", "System Integrator", "Individual"]),
                "industry": random.choice(["Automotive", "Electronics", "Food & Beverage", "Pharmaceuticals", "Aerospace", "General Manufacturing"]) if is_business else "N/A",
                "account_manager": random.choice([e['name'] for e in EMPLOYEES if e['dept'] == "Sales & Marketing"]),
                "lifetime_value": random.randint(10000, 500000) if is_business else random.randint(5000, 50000),
                "total_orders": random.randint(1, 15) if is_business else random.randint(1, 5),
                "status": "Active"
            }
    ...
    def generate_sales_data(self, num_orders=1000):
        ...
        for i in range(num_orders):
            ...
            is_large_order = customer['customer_type'] != "Individual" and random.random() < 0.6
            num_items = random.randint(2, 8) if is_large_order else random.randint(1, 3)
            
            order_total = 0
            
            # Generate order items
            selected_products = random.sample(self.data['products'], min(num_items, len(self.data['products'])))
            for product in selected_products:
                quantity = random.randint(1, 5) if product['category'] in ['Components', 'Software'] else 1
            ...
            if order_total > 50000:
                discount = int(order_total * random.choice([0.05, 0.10]))  # 5-10% volume discount
            elif order_total > 100000:
                discount = int(order_total * random.choice([0.10, 0.15]))  # 10-15% volume discount
            ...
            order = {
                ...
                "payment_method": random.choice(["Purchase Order", "Purchase Order", "Credit Card", "Wire Transfer", "Financing"]),
                "payment_terms": random.choice(["Net 30", "Net 60", "Prepaid", "50/50", "Financing 24mo"]) if customer['customer_type'] != "Individual" else "Prepaid",
                "order_status": random.choice(["Completed", "Completed", "Completed", "Shipped", "In Production", "Cancelled"]),
                "shipping_address": customer['address'],
                "shipping_city": customer['city'],
                "shipping_state": customer['state'],
                "shipping_zip": customer['zip_code'],
                "sales_channel": random.choice(["Direct Sales", "Direct Sales", "Partner", "Online", "Phone"]),
                "sales_rep": random.choice([e['name'] for e in EMPLOYEES if e['dept'] == "Sales & Marketing"])
            }
            orders.append(order)
    ...
    def generate_support_tickets(self, num_tickets=200):
        ...
        for i in range(num_tickets):
            ...
            ticket = {
                ...
                "assigned_to": random.choice([e['name'] for e in EMPLOYEES if e['dept'] == "Customer Service"]),
                "category": random.choice(["Technical Support", "Installation", "Programming", "Maintenance", "Warranty", "Training"]),
                "satisfaction_rating": random.choice([None, None, 3, 4, 4, 5]) if status in ["Resolved", "Closed"] else None
            }
    ...
    def export_csv(self, dataset_name, data, filename):
        ...
        print(f"  ✓ [{self._get_timestamp()}] Exported {len(data)} records to {filename}")
