...
Generate ONLY the HTML content body (no document headers, no <html> or <body> tags)."""
    ...
    def generate_invoices(self, num_invoices=200):
        ...
        completed_orders = [o for o in self.orders if o['order_status'] in ['Delivered', 'Completed']]
        sampled_orders = random.sample(completed_orders, min(num_invoices, len(completed_orders)))
    ...
    def generate_budget_reports(self, num_reports=5):
        ...
        print(f"[{self._get_timestamp()}] Generating {num_reports} quarterly budget reports...")
        
        os.makedirs(os.path.join(self.data_dir, "html_documents/financial"), exist_ok=True)
...
"""
            ...
            content = self._generate_ai_content(f"Quarterly Budget Report - {quarter}", context)
    ...
    def generate_pl_statements(self, num_statements=3):
        ...
        print(f"[{self._get_timestamp()}] Generating {num_statements} P&L statements...")
        
        os.makedirs(os.path.join(self.data_dir, "unstructured/financial"), exist_ok=True)
...
Generate ONLY Markdown content (no HTML, no other formats)."""
    ...
    def generate_campaigns(self, num_campaigns=50):
        ...
        print(f"[{self._get_timestamp()}] Generating {num_campaigns} marketing campaigns...")
        
        campaign_types = ["Email", "Social Media", "Trade Show", "Webinar", "Content Marketing", "PPC", "Direct Mail"]
        target_audiences = [
        ...
        for i in range(num_campaigns):
            ...
            campaign = {
                ...
                "owner": random.choice([e['name'] for e in EMPLOYEES if 'Marketing' in e['dept']]),
                "notes": random.choice([
    ...
    def generate_customer_feedback_forms(self, num_forms=3):
        ...
        print(f"[{self._get_timestamp()}] Generating {num_forms} customer feedback forms...")
        
        os.makedirs(os.path.join(self.data_dir, "html_documents/marketing"), exist_ok=True)
...
"""
    ...
    def generate_lead_tracking_reports(self, num_reports=10):
        ...
        print(f"[{self._get_timestamp()}] Generating {num_reports} lead tracking reports...")
        
        os.makedirs(os.path.join(self.data_dir, "unstructured/marketing"), exist_ok=True)
        
        channels = ["Website", "Trade Shows", "Email Campaigns", "Social Media", "Referrals", "Direct Sales", "Webinars", "Content Downloads"]
...
Generate ONLY Markdown content."""
            ...
            self._print_progress_bar(i + 1, num_reports, "Generating lead tracking reports")
