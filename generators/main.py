...
def generate_structured_data():
    ...
    current_step += 1
    print_progress_bar(current_step, total_steps, "Overall Progress")
    print(f"  ✓ {len(gen.data['products'])} products across {len(PRODUCT_CATEGORIES)} categories\n")
    ...
    print_section_header(f"📊 STRUCTURED DATA COMPLETE (took {elapsed:.1f}s)", start=False)
...
def generate_unstructured_data():
    ...
    elapsed = time.time() - start_time
    print_section_header(f"📝 UNSTRUCTURED DATA COMPLETE (took {elapsed:.1f}s)", start=False)
...
def generate_html_documents():
    ...
    elapsed = time.time() - start_time
    print_section_header(f"📄 HTML DOCUMENTS COMPLETE (took {elapsed:.1f}s)", start=False)
...
def print_summary(structured_data, unstructured_content, documents, data_dir, docs_dir, total_elapsed):
    ...
    print(f"   - Product Docs: {len([d for d in documents if d['category'] == 'Product'])}")
    ...
    total_files = total_unstructured + total_html + len(structured_data) * 2  # CSV + JSON
...
def main():
    ...
    try:
        ...
        print_summary(structured_data, unstructured_content, documents, data_dir, docs_dir, total_elapsed)
